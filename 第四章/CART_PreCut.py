...
def Tree(data: np.array, left_point: list, used_point: dict, dic_index, dic_position):
    ...
    used_point[point] = [-label - 1 for i in range(data[:, point].max() + 1)]
    pre_before = predict(used_point, test_data)

    used_point[point] = []
    for i in range(data[:, point].max() + 1):
        label = get_label(data[data[:, point] == i, :])
        used_point[point].append(-label - 1)

    pre_after = predict(used_point, test_data)
    if pre_after <= pre_before:
        del used_point[point]
        return
    ...
    for i in range(len(test_data)):
        ...
        while True:
            ...
            if new_point < 0:
                ...
                break
            point = new_point
    pre = np.mean([int(i == j[0]) for i, j in zip(result, label)])
    print(pre)
    return pre
