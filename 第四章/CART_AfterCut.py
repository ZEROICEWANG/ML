...
def get_label_fromData(train_data, dic, key):
    p = 0
    n = 0
    for i in range(len(train_data)):
        point = dic[-1][0]
        while True:
            data = train_data[i, point]
            new_point = dic[point][data]
            if new_point < 0:
                if point == key:
                    p += -dic[point][data] - 1
...
def Cut(data, dic, key):
    points = dic[key]
    for i, point in enumerate(points):
        if point < 0:
            continue
        pre_before = predict(dic, test_data)
        if max(dic[point]) < 0:
            label = get_label_fromData(data, dic, point)
            dic_copy = dic[point].copy()
            del dic[point]
            dic[key][i] = -label - 1
            pre_after = predict(dic, test_data)
...
if __name__ == '__main__':
    ...
    used_point = Cut(train_data, used_point, 0)
    print(used_point)
